...
def display_raw_data(city):
    """ display raw data function ask the user if the user would like to see 
    5 rows of the raw data,if yes,data will be displayed and keep checking if 
    additional 5 rows need to be displayes, else "thank you" message will be 
    displayed
    Args:
        (str) city -name of the city to for which the data need to be dsiplayed,
        the name of the city will be taken from get_filter() function.
    Returns:
        if the users enters 'yes': 5 rows of the raw data will be displayed 
        if the user enters 'no': 'Thank you' message will be displayed"""
        
    # load data file into a dataframe
    df = pd.read_csv(CITY_DATA[city])
    
    # create a variable to track the index location in the dataframe
    start_location = 0
    
    # get the user input with 'yes' or 'no' reply for displaying the raw data
    response = input('\nWould you like to see 5 rows of raw data? Enter yes or no.\n')
    
    #create while -except loop to check if the user enters 'yes' or 'no and
    # displays the data in 'yes' case or displays 'thank you' in 'no' case 
    # and break the loop
    while response.lower() == "yes":
        try:   
            for data in df:                
                print(df.iloc[start_location:start_location+5])
                response = input('\nWould you like to see additional 5 rows of raw data? Enter yes or no.\n')
